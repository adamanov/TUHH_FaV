...
class KalmanFilter():
    def __init__(self, name):
        ...
        self.M_1 = np.eye(3, 6)
        rospy.set_param('std_x', 5000) # from measurement
        rospy.set_param('std_y', 0.05)
        rospy.set_param('std_z', 0.00)
        
        self.M_2 = np.zeros((3,6))
        self.M_2[:,3:6] = np.eye(3) 
        rospy.set_param('std_v_x', 50) # from measurement
        rospy.set_param('std_v_y', 50)
        rospy.set_param('std_v_z', 50)

        rospy.set_param('model_std_x', 1) # model
        rospy.set_param('model_std_y', 1)
        rospy.set_param('model_std_z', 1)
        rospy.set_param('model_std_v_x', 1)
        rospy.set_param('model_std_v_y', 1)
        rospy.set_param('model_std_v_z', 1)
    ...
    def prediction_step(self):
        dt = (rospy.Time.now().nsecs - self.last_prediction)/(10**9)
        # print(dt)
        # dt = 0.001
        self.last_prediction = rospy.Time.now().nsecs
        F = np.eye(6)
